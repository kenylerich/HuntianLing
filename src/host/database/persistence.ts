/**
 * Shared huntianling.database persistence. Drivers supply a SQL engine;
 * callers stay on DatabaseService.
 */
...
export class PersistentDatabase implements DatabaseService {
  readonly driver: DatabaseDriver;
  ...
  constructor(options: PersistentDatabaseOptions) {
  ...
  schemaVersion(): number {
    const row = this.engine.get('SELECT MAX(version) AS version FROM schema_migrations');
  ...
  migrate(): number {
    ...
    for (const migration of MIGRATIONS) {
      ...
      this.engine.transaction(() => {
        this.engine.exec(migration.sql);
        this.engine.run('INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)', [
          migration.version,
          Date.now(),
        ]);
      });
  ...
  query(sql: string, params: readonly SqlValue[] = []): readonly Record<string, SqlValue>[] {
    return this.engine.all(sql, params);
  }

  loadBoardSnapshot(): BoardDocument | undefined {
    const row = this.engine.get('SELECT snapshot FROM board_state WHERE id = 1');
    if (row === undefined) return undefined;
    return JSON.parse(String(row.snapshot ?? '')) as BoardDocument;
  }

  saveBoardSnapshot(snapshot: BoardDocument): void {
    const payload = JSON.stringify(snapshot);
    this.engine.transaction(() => {
      this.engine.run('DELETE FROM projects');
      this.engine.run('DELETE FROM team_members');
      this.engine.run('DELETE FROM milestones');
      this.engine.run('DELETE FROM delivery_slices');
      this.engine.run('DELETE FROM workflow_summaries');
      this.engine.run('DELETE FROM delivery_evidence_summaries');
      this.engine.run('DELETE FROM intake_sessions');
      this.engine.run('DELETE FROM intake_messages');
      this.engine.run('DELETE FROM intake_source_documents');
      this.engine.run('DELETE FROM intake_candidates');
      this.engine.run('DELETE FROM audit_events');
      this.engine.run('DELETE FROM work_items');
      this.engine.run('DELETE FROM work_item_links');
      this.engine.run('DELETE FROM acceptance_criteria');
      this.engine.run('DELETE FROM work_item_acceptance_coverage');
      this.engine.run(
        `
      ...
      `,
        [1, snapshot.schemaVersion, payload, Date.now()],
      );

      for (const project of snapshot.projects) {
        this.engine.run('INSERT INTO projects(id, name, payload) VALUES (?, ?, ?)', [
          requireText(project, 'id', 'project'),
          requireText(project, 'name', 'project'),
          JSON.stringify(project),
        ]);
      }

      for (const member of snapshot.teamMembers) {
        this.engine.run('INSERT INTO team_members(id, project_id, payload) VALUES (?, ?, ?)', [
      ...
      for (const milestone of snapshot.milestones) {
        this.engine.run('INSERT INTO milestones(id, project_id, status, payload) VALUES (?, ?, ?, ?)', [
      ...
      for (const summary of snapshot.workflowSummaries) {
        this.engine.run('INSERT INTO workflow_summaries(id, project_id, work_item_id, payload) VALUES (?, ?, ?, ?)', [
      ...
      for (const session of snapshot.intakeSessions) {
        this.engine.run('INSERT INTO intake_sessions(id, project_id, status, payload) VALUES (?, ?, ?, ?)', [
      ...
      for (const message of snapshot.intakeMessages) {
        this.engine.run('INSERT INTO intake_messages(id, session_id, project_id, payload) VALUES (?, ?, ?, ?)', [
      ...
      for (const card of snapshot.cards) {
        ...
        if (parentId !== null) {
          this.engine.run('INSERT INTO work_item_links(from_id, to_id, kind) VALUES (?, ?, ?)', [id, parentId, 'parent']);
        }
        for (const dependencyId of stringArray(card, 'dependencyIds')) {
          this.engine.run('INSERT INTO work_item_links(from_id, to_id, kind) VALUES (?, ?, ?)', [
            id,
            dependencyId,
            'depends',
          ]);
        ...
        criteria.forEach((criterion, index) => {
          ...
          this.engine.run(
            ...
            [id, requireText(row, 'id', 'acceptance criterion'), requireText(row, 'text', 'acceptance criterion'), index],
          );
        });
        for (const criterionId of stringArray(card, 'coversAcceptanceIds')) {
          this.engine.run('INSERT INTO work_item_acceptance_coverage(work_item_id, criterion_id) VALUES (?, ?)', [
            id,
            criterionId,
          ]);
        }
  ...
  getFile(id: string): StoredFile | undefined {
    const row = this.engine.get('SELECT * FROM stored_files WHERE id = ?', [id]);
    if (row === undefined) return undefined;
    return storedFileFromRow(row);
  }

  listFiles(filter: StoredFileFilter = {}): readonly StoredFile[] {
    const rows =
      filter.projectId === undefined
        ? this.engine.all('SELECT * FROM stored_files ORDER BY created_at DESC, id DESC')
        : this.engine.all('SELECT * FROM stored_files WHERE project_id = ? ORDER BY created_at DESC, id DESC', [
            filter.projectId,
          ]);
    return rows.map((row) => storedFileFromRow(row));
  }
  ...
  getCredential(username: string): StoredCredential | undefined {
    const row = this.engine.get('SELECT * FROM credentials WHERE username = ?', [username]);
    if (row === undefined) return undefined;
    return {
      ...
      rotatedAt: row.rotated_at === null || row.rotated_at === undefined ? null : Number(row.rotated_at),
  ...
  upsertAuthUser(record: StoredAuthUser): void {
    this.engine.run(
      ...
      [record.username, record.displayName, record.audience, JSON.stringify(record.projectIds), record.createdAt],
    );
  }

  listAuthUsers(): readonly StoredAuthUser[] {
    const rows = this.engine.all('SELECT * FROM auth_users ORDER BY created_at ASC, username ASC');
  ...
  listAuthEvents(limit = 100): readonly StoredAuthEvent[] {
    const rows = this.engine.all('SELECT * FROM auth_events ORDER BY created_at DESC, id DESC LIMIT ?', [limit]);
    return rows.map((values) => ({
      id: String(values.id ?? ''),
  ...
  listOAuthIdentities(): readonly StoredOAuthIdentity[] {
    const rows = this.engine.all('SELECT * FROM oauth_identities ORDER BY created_at ASC, id ASC');
    return rows.map((values) => ({
      id: String(values.id ?? ''),
  ...
  deleteOAuthIdentity(id: string): void {
    this.engine.run('DELETE FROM oauth_identities WHERE id = ?', [id]);
  }
  ...
  takeOAuthState(state: string): StoredOAuthState | undefined {
    const row = this.engine.get('SELECT * FROM oauth_states WHERE state = ?', [state]);
    this.engine.run('DELETE FROM oauth_states WHERE state = ?', [state]);
    if (row === undefined) return undefined;
    return {
      ...
      codeVerifier: row.code_verifier === null || row.code_verifier === undefined ? null : String(row.code_verifier),
      bindUserId: row.bind_user_id === null || row.bind_user_id === undefined ? null : String(row.bind_user_id),
...
function storedFileFromRow(row: Record<string, SqlValue>): StoredFile {
  return {
    ...
    extractedTextPath:
      row.extracted_text_path === null || row.extracted_text_path === undefined ? null : String(row.extracted_text_path),
    createdAt: Number(row.created_at ?? 0),
  };
}
